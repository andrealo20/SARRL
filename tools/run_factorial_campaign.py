...
source state, committed-artifact seed scan, exclusive lock, session-tagged
at-least-once journal, canonical log reloaded for the analysis, completion
marker hashing every output. Every cell runs on the MuJoCo plant with the
v2.0 options and measured state; the reproduction block reruns the two v2.0
arms on the first hundred v2.0 decision seeds per scenario and every
retained field must match the v2.0 log.
"""
...
def main() -> int:
    ...
    print(f"  reproduction of retained v2.0 rows: {check['matched']}/{check['compared']}")
    return 0
